...
public class AryLinkedList<E> {
	...
	private int getInsertIndex() {
		if (deleted == NULL) {		// 삭제할 레코드가 없음
			...
			int rec = deleted;				// free 리스트에
			deleted = n[rec].dnext;			// 머리 rec을 꺼냄
			return rec;
		}
	}
	
	// record idx 를 free 리스트에 등록
	private void deleteIndex(int idx) {
		if (deleted == NULL) {					// 삭제할 레코드가 없음
			deleted = idx;						// idx 를 free 리스트의
	...
	public void addFirst(E obj) {
		...
		if (rec != NULL) {
			head = crnt = rec;				// 인덱스 rec 인 record 에 삽입
			n[head].set(obj, ptr);
		}
	}
	...
	// 선택 노드를 하나 뒤쪽으로 이동
	public boolean next() {
		if (crnt == NULL || n[crnt].next == NULL) {			// 현재 커서가 존재하지 않는 인덱스거나, 다음 커서가 존재하지 않는 인덱스일 경우	
			return false;
		}
		crnt = n[crnt].next;
		return true;
