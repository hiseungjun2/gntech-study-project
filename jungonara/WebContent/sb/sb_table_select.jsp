...
<%
	...
	ArrayList<sbDto> sb = (ArrayList<sbDto>)request.getAttribute("sbselectlist");
	int count = (int)request.getAttribute("sbselectcount");
	String open = (String)request.getAttribute("sbopenselect");
	String input = (String)request.getAttribute("sbinputselect");
%>
<!-- *************************** 모달 ******************************** -->
	<div class="modal fade bs-example-modal-lg" id="sbModal" role="dialog">
        <div class="modal-dialog modal-lg">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header" style="padding:15px 50px;">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4><span class="glyphicon glyphicon-pencil"></span> 게시물 등록</h4></div>
                <form role="form" action="./sbinsert" method="post">
                    <div class="modal-body" style="padding:40px 50px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="sb_title" class="form-control" placeholder="글 제목을 입력하세요">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                        	...
                        	<input type="text" name="sb_price" class="form-control" placeholder="ex) 1000000">
                        </div>
                        <div class="form-group">
                            <label for="sb_contents"><span class="glyphicon glyphicon-pencil"></span> 글 내용</label>
                            <textarea class="form-control" rows="10" placeholder="내용을 입력해주세요" name="sb_contents"></textarea>
                        </div>
                    </div>
    ...
    <div class="container">
        <div id="basic_table">
            <div class="container-body">
                <h2>팝니다<small> 팝니다 게시글 작성 및 검색이 가능합니다.</small></h2>
				<button type="button" class="btn btn-default pull-right" data-toggle="modal" data-target="#sbModal" >글쓰기</button>
					<div>
					...
					<div>
						<nav style="display:inline">
							<ul class="pagination">
								...
								<% for(int i = 1; i<=Math.ceil(count/12.0);i++) { %>
								<li><a href="/jungonara/sbselect?pageNum=<%=i%>&optionSelect=<%=open %>&inputSelect=<%=input%>"><%=i %></a></li>		<!-- 페이지네이션 선언 -->
								<%} %>
								<li><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
							</ul>
				        </nav>	
				        <form action="./sbselect" method="get" class="form-inline pull-right" style="margin-top : 20px;">
				        <input type="hidden" name="pageNum" value="1">
				        	<div class="form-group">
				        		<div class="row col-xs-2">
				        			<select id="optionSelect" class="form-control" name="optionSelect">
				        	...
				        	<div class="form-group" >
				        		<div class="row col-xs-3">
				        			<input name="inputSelect" id="inputSelect" type="text" class="form-control" placeholder="검색어 입력">
				        		</div>
				        	</div>
				        	<button id="btnSelect" type="submit" class="btn btn-default">검색</button>
				        </form>
					</div>
            </div>
        </div>
...
<script>
	$(document).ready(function() {
		$("#sb tbody tr td").click(function() {
			...
			$(location).attr("href", "/jungonara/sbselectinfo?num=" + num);
		})
	})
</script>
</html>